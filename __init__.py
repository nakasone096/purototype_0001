...
class StageManager:
    ...
    def reset_scene_for_chapter(context):
        ...
        if ch == 1:
            ...
            # B1修正: St1はキューブを「選択する」課題なので初期状態は非選択にする
            if props.current_stage == 1:
                cube.select_set(False)
                bpy.context.view_layer.objects.active = None

        elif ch == 2:
            # B2修正: Ch1から来たとき変形されたキューブが残るのでトランスフォームをリセット
            for obj in bpy.data.objects:
                if obj.type == 'MESH' and obj.name == "Cube":
                    ...
                    break
            # B3修正: 視点を指定位置にリセットしてから初期値として記録する
            # カメラ位置 x=10 y=-9 z=3 / 回転 x=80 y=0 z=45 deg
            ...
            # T9修正: St1はプレイヤー自身がスカルプトモードに切り替える課題のため
            # セットアップではオブジェクトモードのまま置いておく
            # St2以降はすでにスカルプトモードに入っているはずなのでそのまま
            ...
            # Ch6: マテリアル
            # ノードを何度も作り直す手間を省くため、オブジェクトのリセットは行わない
            # 初回セットアップ時のみキューブがなければ作成する
            try:
                bpy.ops.object.mode_set(mode='OBJECT')
            except Exception:
                pass
...
class TUTORIAL_OT_validate_stage(Operator):
    ...
    def execute(self, context):
        ...
        if ok:
            ...
            # Ch5はスカルプト形状、Ch6はマテリアルノード、Ch3 St2はundo_countが消えるためリセットしない
            if props.current_chapter in (5, 6):
                pass
