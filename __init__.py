        ...
        if chapter_num == 3:
            stages = {
                ...
                2: {"title": "第3章: オブジェクト操作", "name": "ステージ2: 元に戻す",
                    "description": "Ctrl+Z で操作を元に戻してください",
                    "details": "① G → X → 3 → Enter でキューブを移動\n"
                               "② Ctrl+Z を押して元の位置に戻そう"},
    ...
    def validate_stage(context):
        ...
        if ch == 3:
            ...
            if st == 1:
                ...
                return False, "❌ Shift+A でオブジェクトを追加してください", "NO_OBJECT_ADDED", ["Shift+A → メッシュ → 立方体など"]

            # --- 新St2: Ctrl+Z で元に戻す ---
            if st == 2:
                if props.undo_count > 0:
                    return True, "✓ 元に戻せました", "OK", []
                return False, "❌ Ctrl+Z を押してください", "UNDO_NOT_DONE", ["G → X → 3 → Enter でキューブを移動", "その後 Ctrl+Z で元に戻そう"]
            ...
            if st == 3:
                ...
                return False, "❌ オブジェクトを削除してください", "OBJECT_NOT_DELETED", ["オブジェクトを選択 → X キー → 「削除」"]
        ...
        try:
            ...
            with open(out_csv, "w", encoding="utf-8-sig", newline="") as f:
                ...
                w.writerow([
                    ...
                    "undo_count", "survey_difficulty", "reason_sequence",
                ])
...
class TUTORIAL_OT_next_stage(Operator):
    ...
    def execute(self, context):
        ...
        max_stages_per_chapter = {1: 4, 2: 4, 3: 9, 4: 4, 5: 5, 6: 1}
...
class TUTORIAL_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        if props.stage_complete:
            svy = layout.box()
            svy.label(text="✓ クリア！ 難しさを教えてください", icon='CHECKMARK')
            svy.label(text="1 = 簡単  ←→  5 = 難しい")
            row = svy.row(align=True)
            for i in range(1, 6):
                op = row.operator(
                    "tutorial.rate_difficulty",
                    text=f"★{i}",
                    depress=(props.survey_difficulty == i),
                )
                op.difficulty = i
            if props.survey_difficulty > 0:
                svy.label(text=f"回答済み: ★{props.survey_difficulty}")
