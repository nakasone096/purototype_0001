...
class StageManager:
    ...
    def apply_hint_escalation(hints, failed_validate_count: int, difficulty_mode: str = 'NORMAL'):
        ...
        if difficulty_mode == 'EASY':
            return hints[:3]          # 最初から全ヒント表示
        if difficulty_mode == 'HARD':
            return []                 # ヒントなし
        # NORMAL: 失敗するたびに段階的に表示
    ...
    def validate_stage(context):
        ...
        if ch == 3:
            ...
            if st == 2:
                # Undoスタックの競合を避けるため自動判定せず手動確認のみで通過
...
class TUTORIAL_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        at_end   = (props.current_chapter == 6 and props.current_stage == max_st)
        left = nav.row()
